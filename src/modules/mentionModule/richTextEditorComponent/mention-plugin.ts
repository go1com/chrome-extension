...
export const TCMention = MediumEditor.Extension.extend({
  ...
  initMentionPanel() {
    ...
    if (this.extraPanelClassName) {
      el.classList.add(this.extraPanelClassName);
    }
  ...
  hidePanel(isArrowTowardsLeft) {
    ...
    const extraActivePanelClassName = this.extraActivePanelClassName;
  ...
  activatePanel() {
    ...
    if (this.extraActivePanelClassName) {
  ...
  handleSelectMention(selectedText) {
    if (selectedText) {
      if (!selectedText.startsWith(this.trigger)) {
        selectedText = `${this.trigger}${selectedText}`;
      }
