...
import {IRestClientService} from "./IRestClientService";
...
@injectable()
export class DefaultRestClientService implements IRestClientService {
  getUrls: any[] = [];
  ...
  singleGet<T>(url: string, customHeaders?: any) {
    const existingRequest = this.getUrls.find(getRequest => getRequest.url === url);
    if (existingRequest) {
